import React, {
  ...
  const PoliceCar: React.FC<{
    ...
    useFrame(() => {
      if (target.current && ref.current) {
        ...
        direction.subVectors(target.current.position, ref.current.position).normalize()
        // Move police car toward player slowly.
  ...
  const BonusCoin: React.FC<{ position: [number, number, number] }> = ({
    position,
  }) => {
    const ref = useRef<any>()
  ...
  const Hazard: React.FC<{ position: [number, number, number] }> = ({
    position,
  }) => {
    // For simplicity, we render a flat textured plane.
    return (
  ...
  const RaceTrack: React.FC = () => {
    ...
    // Animate the track color to simulate changing road scenarios.
  ...
  const GameController: React.FC<{
    ...
    // Modify player's speed when slowed.
    const baseSpeed = 0.5
    ...
    useFrame(() => {
      if (playerRef.current) {
        ...
        obstacles.forEach((obs) => {
          ...
          if (distance < 2) {
            // Process collision based on obstacle type.
            if (obs.type === "coin") {
              onCollectBonus(5)
              obs.collected = true
              ...
              if (!invincible) {
                // Slow down the car temporarily and show a scolding message.
        ...
        // Clean up collected or passed obstacles.
        setObstacles((prev) =>
  ...
  const CarRacingGame: React.FC = () => {
    ...
    // When a powerup (invincibility) is collected, enable invincibility.
    useEffect(() => {
      // Check if any obstacle of type "powerup" was collected.
